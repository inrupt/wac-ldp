...
The ACL manager implements the 'wac' in 'wac-ldp'. It has direct query access to the quads in the triple store, so that it doesn't need to be aware of any RDF serializations. This is a nice separation of concerns that other Solid pod server architectures like https://github.com/solid/node-solid-server and https://github.com/rubenverborgh/solid-server-architecture lack.
